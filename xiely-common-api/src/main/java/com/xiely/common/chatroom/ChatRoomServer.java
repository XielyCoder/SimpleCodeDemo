package com.xiely.common.chatroom;
...
public class ChatRoomServer
{
    ...
    private void dealWithSelectionKey(ServerSocketChannel server, SelectionKey sk) throws IOException
    {
        ...
        if (sk.isReadable())
        {
            ...
            {
                //相当于自定义协议格式，与客户端协商好
                String USER_CONTENT_SEPARATOR = "#@#";
                String[] arrayContent = content.toString().split(USER_CONTENT_SEPARATOR);
                //注册用户
                ...
                {
                    String name = arrayContent[0];
                    String message = content.substring(name.length() + USER_CONTENT_SEPARATOR.length());
